...
AJAX('GET',storage,REYURNURL(ADDBANCOACH),{"regionId":"","rank":"","coachSex":"","serviceType":1},addCoachList);
...
$('.qu').change(function () {
    ...
    AJAX('GET',storage,REYURNURL(ADDBANCOACH),{"regionId":regionId,"rank":rank,"coachSex":coachSex,"serviceType":1},addCoachList);
});
$('.sex').change(function () {
    var regionId = $('.qu').val();
    var coachSex = $('.sex').val();
...
function adddanCoach(json) {
  if(json.code == 0){
    alert("添加成功");
    window.location.href="shuang.html";
  }else{
    alert("添加失败");
  }
}
