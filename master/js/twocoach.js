...
function coachInfo(json) {
  if(json.code == 0){
    var data = json.data;
      console.log(data);
      $('.coachName').val(data.aloneName);
         $('.coach').val(data.aloneReserve);
        $('.coac').val(data.maxCount);
          $('.coachName').val(data.aloneName);
      ...
      $('.ban option').each(function (index,value) {
        if($(value).attr('value') == data.skateboart){
          $(this).attr("selected",true);
        }
      });

      $('.coachType option').each(function (index,val) {
        if($(val).attr('value') == data.rankId){
          $(this).attr("selected",true);
        }
      });
      ...
      $(function(){
        ...
        //给ueditor插入值
        function setContent(){
            ue.execCommand('insertHtml', data.aloneIntro);
            ue2.execCommand('insertHtml', data.aloneGlory);
...
AJAX('GET',storage,'/api/admin/alone/echo',{"aloneId":id},coachInfo);
...
var arr=lableArr ;
    ...
    if(coachName == '' || portrait == '' ){
      ...
      AJAX('POST',storage,'/api/admin/alone/updatealone',obj,addCoach);
    }
