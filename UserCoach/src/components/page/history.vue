...
-->
<!-- <img src="../../assets/zai.gif" v-show="loading"> -->
<ul class="listcon" v-show="arr.length != 0">
  <li v-for="value in arr"  @click="ban(value.cid,value.monomerid,value.serviceType,value.type,value.aloneid)">
    <div class="list">
      <img class="img" :src="value.curl+'_w100'" alt="">
      <p class="p1">
        {{value.cname}}
      </p>
      <p class="p2">
...
<script>
  ...
  export default {
    ...
    this.$http.get(Config.api.sinf,{params:{regionId:this.id, serviceType:0}}).then(
      function(response){
...
var h=true;
        ...
        that.cun.car=ay
           ...
           //请求搜索接口
             if(window.location.hash='#/home/history'){
            var timep=10;
            var that=this
        ...
        methods:{
          ...
          ban(id,gid,type,one,oneid){
            ...
            if(one==1){
          this.$router.push('/one?id='+oneid);//一对一教练
            }else{
               this.$router.push('/Xqy?id='+id+'&type='+gid);//单板教练
            }
