<template>
 ...
 <div id="box">
    ...
    <ul class="ziji">
     ...
     <p class="zij"></p>
    <div style="display: block;" v-html="ry">
...
退款会在1 - 3个工作日内返还至您的支付账户。
  ...
  <p class="p">4. 如有疑问请拨打客服电话400-811-7555（在线时间：8:00 - 18:00）</p>
    ...
    <div class="most">
      <div style="color:#ccc;font-size:0.7rem;text-align:center;padding-bottom:1rem;" v-show="evaluates.length == 0">
        <div class="comNum" >
          <section class="num-o">
...
//获取到教练基本信息
    this.$http.get(Config.api.one,{params:{aloneId:cid}}).then(
      function(response){
      // console.log(response.data.data)
        this.json=response.data.data
         ...
         this.endt=this.json.enddate
          this.ry=response.data.data.glory
          this.rid=this.json.rid
          if(this.linshi!='111'){
console.log(this.rid)
      this.$http.get(Config.api.lookins,{params:{type:2,'id':this.rid},
          ...
          localStorage.setItem('ds',response.data.data.serviceType);//单双带到下个页面
...
//获取用户评价
var commentInfo = Config.api.onepj;// 用户评价接口
this.$http.get(commentInfo,{params:{aloneId:cid}}).then(function (response) {
...
console.log(this.code)
 ...
 for(var i=0;i<this.date.length;i++){
   ...
   this.$http.get(Config.api.onetiem,{params:{aloneId:this.cid}}).then(
      function(response){
